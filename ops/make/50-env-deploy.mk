...
deploy-prod:
	...
	ssh -p "$$DEPLOY_PORT" "$$DEPLOY_USER@$$DEPLOY_HOST" "set -euo pipefail; cd '$$DEPLOY_DIR'; trap 'rm -f ops/env/$$DEPLOY_ENV_FILE' EXIT; sudo docker pull \"$$APP_IMAGE_REF\"; sudo docker pull \"$$INBOUND_IMAGE_REF\"; sudo APP_IMAGE=\"$$APP_IMAGE_REF\" INBOUND_SMTP_IMAGE=\"$$INBOUND_IMAGE_REF\" docker compose --project-name \"$$DEPLOY_INSTANCE_NAME\" --env-file \"ops/env/$$DEPLOY_ENV_FILE\" -f \"ops/docker-compose.prod.yml\" up -d --force-recreate; sudo APP_IMAGE=\"$$APP_IMAGE_REF\" INBOUND_SMTP_IMAGE=\"$$INBOUND_IMAGE_REF\" docker compose --project-name \"$$DEPLOY_INSTANCE_NAME\" --env-file \"ops/env/$$DEPLOY_ENV_FILE\" -f \"ops/docker-compose.prod.yml\" exec -T app php bin/console doctrine:migrations:migrate -n; sudo APP_IMAGE=\"$$APP_IMAGE_REF\" INBOUND_SMTP_IMAGE=\"$$INBOUND_IMAGE_REF\" docker compose --project-name \"$$DEPLOY_INSTANCE_NAME\" --env-file \"ops/env/$$DEPLOY_ENV_FILE\" -f \"ops/docker-compose.prod.yml\" ps"
